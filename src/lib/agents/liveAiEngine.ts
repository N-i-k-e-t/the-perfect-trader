...
/**
 * LiveAiEngine: Handles the actual LLM calls used across The Perfect Trader
...
export async function askAi(prompt: string, isJson: boolean = false) {
    if (!genAI) {
        console.warn("The Perfect Trader AI: GEMINI_API_KEY missing in server. Reverting to Simulated Model.");
    ...
    try {
        ...
        console.error("The Perfect Trader Live Logic Error:", e);
        return simulateAi(prompt, isJson);
    }
}
...
function simulateAi(prompt: string, isJson: boolean) {
    ...
    return "The Perfect Trader Agent: AI bridge is active. Please add GEMINI_API_KEY to your environment to enable real-time analysis.";
}
