...
export default function DailyStateCheck() {
    ...
    const handleComplete = () => {
        ...
        const lockedRules = rules.filter((r) => r.isActive !== false);
        completePreSession();
        track('session_pre_plan_completed', 'session', {
            session_date: todayDate,
            emotional_baseline: session.emotionalBaseline,
            rules_locked_count: lockedRules.length,
            time_to_complete_ms: Date.now() - formOpenedAt.current,
        });
    };
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-white flex flex-col p-6 pb-[calc(env(safe-area-inset-bottom)+20px)] sheet-scroll">
                        ...
                        >
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {['neutral', 'anxious', 'greedy', 'fearful', 'calm', 'distracted'].map(
                                        (mood) => (
                                            <button
                                                key={mood}
                                                onClick={() =>
            ...
            <footer className="pt-8 mb-4">
                ...
                <p className="text-[11px] font-bold text-gray-300 text-center mt-6 uppercase tracking-[0.2em]">
                    The Perfect Trader Discipline Shield Active
                </p>
            </footer>
        </div>
    );
